...
class output_pin_list_t:public peripheral_list_t<output_pin_t>
{
  public:
    ...
    inline bool on_add(output_pin_t& output,json_ro_t& json,const std::string& path)
    {
      output.pin=std::stoul(json[path]);
    ...
    inline void on_set(output_pin_t& output,json_ro_t& json,const std::string& path)
    {
      uint32_t value=std::stoul(json[path]);
      output.value=value;
